...
class Mission(db.Model):
    ...
    def save_mission(self, name, release_date, endpoint, mission_state, crew, playload, duration, cast, status):
        try:
            add_banco = Mission(name, release_date, endpoint, mission_state, crew, playload, duration, cast, status)
            print(add_banco)
    ...
    def update_mission(self, id, name, release_date, endpoint, mission_state, crew, playload, duration, cast, status):
        try:
            db.session.query(Mission).filter(Mission.id==id).update({"name":name,"release_date" : release_date , "endpoint" : endpoint, "mission_state":mission_state})
        except Exception as e:
            print(e)
